...
func TestPilot(t *testing.T) {
	...
	config.GetEConfig().Init("../../enova/template/deployment/docker-compose/pilot/conf/settings.json")
	...
	req, _ := http.NewRequest("POST", "/api/pilot/v1/docker/deploy", bytes.NewBuffer(bodyBytes))
	...
	// query task
	req, _ = http.NewRequest("GET", fmt.Sprintf("/api/pilot/v1/docker/deploy?task_name=%s", taskName), nil)
	w = httptest.NewRecorder()
	d.GetEngine().ServeHTTP(w, req)
	assert.Equal(t, http.StatusOK, w.Code)
	...
	req, _ = http.NewRequest("GET", fmt.Sprintf("/api/pilot/v1/task/detect/history?task_name=%s", taskName), nil)
	w = httptest.NewRecorder()
	d.GetEngine().ServeHTTP(w, req)
	assert.Equal(t, http.StatusOK, w.Code)
	if err := json.Unmarshal(w.Body.Bytes(), &resp); err != nil {
	...
	req, _ = http.NewRequest("DELETE", fmt.Sprintf("/api/pilot/v1/docker/deploy?task_name=%s", taskName), nil)
	w = httptest.NewRecorder()
	d.GetEngine().ServeHTTP(w, req)
	assert.Equal(t, http.StatusOK, w.Code)
	if err := json.Unmarshal(w.Body.Bytes(), &resp); err != nil {
